...
const PHONE_NUMBER = "+917635052209";

async function sendAlertSms({ phone, title, riskMessage, location }) {
  const res = await fetch("http://localhost:3000/send-alert-sms", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ phone, title, riskMessage, location }),
  });
...
const Dashboard = () => {
  ...
  const handleTakeAction = async ({ title, riskMessage, location }) => {
    setSmsStatus("");
    try {
      await sendAlertSms({ phone: PHONE_NUMBER, title, riskMessage, location });
      setSmsStatus("SMS sent successfully!");
    } catch (err) {
      setSmsStatus("Failed to send SMS: " + err.message);
    }
  };
  ...
  return (
    <div className="h-screen w-full flex">
      ...
      <div className="w-[85%] h-full p-5 rounded-lg bg-gray-50">
        <div className="heading h-[20%] w-full bg-gradient-to-r from-blue-300 via-green-300 to-green-400 rounded-2xl p-10 flex flex-col justify-center relative overflow-hidden">
          ...
          <p className="text-lg text-white mb-4 drop-shadow">
            Protecting Northeast India communities through AI-powered water
            quality monitoring and early disease detection
          </p>
        ...
        <div className="info h-[10%] w-full  mt-4 flex justify-evenly">
          ...
          <div className="flex rounded-lg p-2 w-60 bg-gradient-to-b from-gray-10 to-gray-200 shadow-lg ">
            <div className="h-full w-10 flex items-center">
              <TriangleAlert
                size={30}
                className="text-amber-300 bg-amber-100 rounded-sm"
              />
            </div>
            <div className="flex flex-col  justify-center">
          ...
          <div className="flex rounded-lg p-2 w-60 bg-gradient-to-b from-gray-10 to-gray-200 shadow-lg  ">
            <div className="h-full w-10 flex items-center">
              <ChartColumnIncreasing
                size={30}
                className="text-blue-300 bg-blue-100 rounded-sm"
              />
            </div>
            <div className="flex flex-col  justify-center">
          ...
          <div className="flex rounded-lg p-2 w-60 bg-gradient-to-b from-gray-10 to-gray-200 shadow-lg  ">
            <div className="h-full w-10 flex items-center">
              <User
                size={30}
                className="text-emerald-500 bg-emerald-100 rounded-sm"
              />
            </div>
            <div className="flex flex-col  justify-center">
        ...
        <div className="sources h-[60%] w-full  flex">
          ...
          <div className="h-135 w-[30%]  rounded-xl mt-2 p-4 shadow-lg">
            ...
            {/* Example alert cards with Take Action handler */}
            <div className="alert h-55 w-[100%]  mt-5 rounded-lg flex flex-col items-start p-4 shadow-lg">
              <div className="flex ">
                <h1 className="text-[15px] font-bold mb-2 mr-5">
                  High Cholera Risk Detected
                </h1>
                <h1 className="text-sm font-bold bg-red-100 px-[5px] rounded-lg text-red-800 ">
                  HIGH
                </h1>
              </div>
              ...
              <p className="text-xs mb-4">
                Recent water quality analysis indicates a significant increase
                in cholera risk due to contaminated sources.
              </p>
              <button
                className="bg-cyan-600 text-white px-2 py-1  rounded hover:bg-cyan-800 transition-all text-sm"
                onClick={() =>
                  handleTakeAction({
            ...
            {smsStatus && (
              <div className="text-green-700 mt-2">{smsStatus}</div>
            )}
          </div>
        </div>
      </div>
